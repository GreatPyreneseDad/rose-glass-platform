"""
Rose Glass Database - PostgreSQL version for Vercel deployment
Supports Vercel Postgres and other PostgreSQL databases via DATABASE_URL
"""
...
class RoseGlassDB:
    ...
    def log_exchange(
        ...
        cursor.execute(
            "INSERT INTO conversations (id, model) VALUES (%s, %s) ON CONFLICT (id) DO NOTHING",
            (conversation_id, model)
        )

        # Update exchange count
        cursor.execute(
            "UPDATE conversations SET total_exchanges = total_exchanges + 1 WHERE id = %s",
            (conversation_id,)
        )
    ...
    def get_conversation(self, conversation_id: str) -> Optional[Dict]:
        ...
        # Get exchanges
        cursor.execute(
            "SELECT * FROM exchanges WHERE conversation_id = %s ORDER BY created_at",
            (conversation_id,)
        )
        ...
        cursor.execute(
            "SELECT * FROM perception_timeseries WHERE conversation_id = %s ORDER BY timestamp",
            (conversation_id,)
        )
    ...
    def get_recent_conversations(self, limit: int = 50) -> List[Dict]:
        ...
        cursor.execute("""
            ...
            LEFT JOIN LATERAL (
                SELECT * FROM perception_timeseries
                WHERE conversation_id = c.id
                ORDER BY id DESC LIMIT 1
            ) p ON true
    ...
    def get_perception_stats(self, hours: int = 24) -> Dict:
        ...
        return {
            ...
            "averages": {
                "psi": round(float(row["avg_psi"] or 0), 3),
                "rho": round(float(row["avg_rho"] or 0), 3),
                "q": round(float(row["avg_q"] or 0), 3),
                "f": round(float(row["avg_f"] or 0), 3),
                "coherence": round(float(row["avg_coherence"] or 0), 3)
            },
            "state_counts": {
